...
const ListingPreview = () => {
  ...
  const [isShouldBeExcluded, setIsShouldBeExcluded] = useState(false);
  ...
  useEffect(() => {
    if (oneListing) {
      ...
      const propertyType = rowGroup[1].find(
        (feature) => feature.name === "value--col"
      )?.value;
      if (propertyType && excludedPropertyTypes.includes(propertyType)) {
        rowGroup[1].splice(3, 2);
        rowGroup[3].splice(3, 2);
        setIsShouldBeExcluded(true);
      } else {
        setIsShouldBeExcluded(false);
      }
    }
  }, [oneListing]);
                            ...
                            {propertyDetails.map((group, index) => (
                              ...
                              >
                                <div className="property-highlights-table-row">
                                  {group.map((feature, i) => {
                                    // Check if we should exclude certain rows
                                    console.log("feature: ", feature);

                                    if (isShouldBeExcluded) {
                                      // Exclude specific features based on their values
                                      if (
                                        feature.value === "Bedroom" ||
                                        feature.value === "Bathroom" ||
                                        feature.value === "No. of Floors" ||
                                        feature.value === "Car Parking"
                                      ) {
